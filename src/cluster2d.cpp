...
#include "lidar_instance_segmentation_tvm/cluster2d.h"
  ...
  bool use_all_grids_for_clustering)
{
  ...
  const float * non_object_pt_data = inferred_data.get() + siz_ * 4;
  const float * pred_car_pt_data = inferred_data.get() + siz_ * 5;
  const float * pred_cyclist_pt_data = inferred_data.get() + siz_ * 6;
  const float * pred_pedestrian_pt_data = inferred_data.get() + siz_ * 7;
  ...
  for (int row = 0; row < rows_; ++row) {
    for (int col = 0; col < cols_; ++col) {
      ...
      // node->is_object = (use_all_grids_for_clustering || nodes[row][col].point_num > 0) &&
      //                   (1.0 - *(non_object_pt_data + grid) >= objectness_thresh);
      // node->is_object = (use_all_grids_for_clustering || nodes[row][col].point_num > 0) &&
      //                   (*(pred_car_pt_data + grid) >= 0.4 ||
      //                   *(pred_cyclist_pt_data + grid) >= 0.6 ||
      //                   *(pred_pedestrian_pt_data + grid) >= 0.3);
...
void Cluster2D::filter(const std::shared_ptr<float> & inferred_data)
{
  ...
  const float * non_empty_pt_data = inferred_data.get() + siz_ * 4;
  ...
  for (size_t obstacle_id = 0; obstacle_id < obstacles_.size(); obstacle_id++) {
    ...
    for (int grid : obs->grids) {
      ...
      // score += static_cast<double>(1.0 - non_empty_pt_data[grid]);
      vec_x += static_cast<double>(heading_pt_x_data[grid]);
      vec_y += static_cast<double>(heading_pt_y_data[grid]);
  ...
  const Obstacle & in_obstacle, const std_msgs::Header & in_header)
{
  ...
  resulting_object.object.shape.dimensions.x = std::cos(in_obstacle.heading);
  resulting_object.object.shape.dimensions.y = std::sin(in_obstacle.heading);
  ...
  resulting_object.object.shape.type = autoware_perception_msgs::Shape::BOUNDING_BOX;
  resulting_object.object.shape.dimensions.x = 4;
  resulting_object.object.shape.dimensions.y = 2;
...
std::vector<int> Cluster2D::getIdImage() { return id_img_; }